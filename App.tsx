        ...
        <div className="p-6 border-b border-slate-700">
          ...
          <div className="mt-4 p-3 bg-slate-800 rounded-lg border border-slate-700">
            ...
            <div className="text-sm font-bold flex items-center justify-between">
              <span>{currentUser.name} <span className="text-indigo-400 text-[10px]">{currentUser.role}</span></span>
              <button onClick={handleLogout} className="text-[10px] text-rose-400 hover:text-rose-300">로그아웃</button>
            </div>
          </div>
        </div>
        <nav className="flex-1 p-4 space-y-1">
          <button onClick={() => setCurrentView(View.DASHBOARD)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.DASHBOARD ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📊 대시보드</button>
          <div className="pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase px-4">결의서 관리</div>
          <button onClick={() => setCurrentView(View.INCOME)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.INCOME ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📥 수입결의서 작성</button>
          <button onClick={() => setCurrentView(View.EXPENSE)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.EXPENSE ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📤 지출결의서 작성</button>
          <button onClick={() => setCurrentView(View.HISTORY)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.HISTORY ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📜 결의내역 및 전자결재</button>
          <div className="pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase px-4">재정 보고</div>
          <button onClick={() => setCurrentView(View.JOURNAL)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.JOURNAL ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📒 금전출납부</button>
          <button onClick={() => setCurrentView(View.REPORT)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.REPORT ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>📑 연간 결산보고</button>
          <div className="pt-4 pb-2 text-xs font-semibold text-slate-500 uppercase px-4">시스템</div>
          <button onClick={() => setCurrentView(View.SETTINGS)} className={`w-full text-left px-4 py-3 rounded ${currentView === View.SETTINGS ? 'bg-indigo-600' : 'hover:bg-slate-800'}`}>⚙️ 시스템 설정</button>
        </nav>
